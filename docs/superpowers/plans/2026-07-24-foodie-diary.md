...
- **DEVIATION (2026-07-24) — explicit package products:** TCA re-exports `Dependencies`/`DependenciesMacros` at compile time only; under Tuist native Xcode package integration their runtime witnesses (`DependencyKey`/`DependencyValues`, `@DependencyClient`'s `Unimplemented`) and the transitive `IssueReporting` are not auto-linked. So `Project.swift` links these products explicitly: `ClientKit` → `ComposableArchitecture, Dependencies, DependenciesMacros, IssueReporting`; `FeatureKit` → `ComposableArchitecture, Dependencies, IssueReporting`. (Constants `tca`, `dependencies`, `depMacros`, `issueReporting`.) Verified: builds + tests link.
